...
| android | a_message | a message |     |     | 一条消息 | メッセージ |
...
| android | Card | Card | Kartu | Kad | 卡片  | カード |
| android | changed_disappearing_message_settings | %1$s changed disappearing message settings. |     |     | %1$s 修改了限时消息设置 |     |
...
| android | conversation_delete_title | Delete chat: %1$s |     |     | 删除会话：%1$s | チャットを削除する：%1$s |
...
| android | disappearing_option_1 | 30 seconds |     |     | 30 秒 |     |
| android | disappearing_option_2 | 10 minutes |     |     | 10 分 |     |
| android | disappearing_option_3 | 2 hours |     |     | 2 小时 |     |
| android | disappearing_option_4 | 1 day |     |     | 1 天 |     |
| android | disappearing_option_5 | 1 week |     |     | 1 周 |     |
...
| android | Duration_is_too_short | Duration is too short | Durasi terlalu pendek | Jangka masa terlalu pendek | 时间太短 | 期間が短すぎます |
...
| android | error_server_5xx_code | Server is under maintenance: %1$d | Server sedang dalam pemeliharaan: %1$d | Pelayan sedang dalam penyelenggaraan: %1$d | 服务器出错，请稍后重试: %1$d | サーバーメンテナンス中: %1$d |
...
| android | Exit | Exit | Keluar | Keluar | 退出  | 退出  |
...
| android | Follow_system | Follow System | Otomatis | Ikut Sistem | 跟随系统 | システム設定に従う |
...
| android | group_cant_send | You can't send messages to this group because you're no longer a participant. | Anda tidak dapat mengirim pesan ke grup ini karena Anda bukan lagi peserta. | Anda tidak dapat menghantar mesej kepada kumpulan ini kerana anda bukan lagi peserta. | 您不能发送消息，因为您已经不再是此群组成员。 | 参加者ではないため、このグループにメッセージを送ることができません。 |
...
| android | group_pop_menu_message | Message %1$s | Kirim pesan ke %1$s | Mesej %1$s | 发送消息至 %1$s | メッセージを送信 |
| android | group_pop_menu_remove | Remove %1$s | Hapus %1$s | Alih keluar %1$s | 移除 %1$s | グループから退会させる |
...
| android | landing_invitation_dialog_content | We will send a 4-digit code to your phone number %1$s, please enter the code in next screen. | Kami akan mengirim kode 4 digit ke nomor telepon Anda %1$s, harap masukkan kode tersebut pada layar berikutnya. | Kami akan menghantar kod 4 digit ke nombor telefon anda %1$s, sila masukkan kod di skrin seterusnya. | 我们将发送4位验证码到手机 %1$s, 请在下一个页面输入 | 4桁のコードを電話番号%1$sに送信します。次の画面でコードを入力してください。 |
...
| android | loading_time | System time is unusual, please continue to use again after correction | Waktu sistem tidak normal, silakan gunakan lagi setelah perbaikan dilakukan | Waktu sistem tidak biasa, sila terus gunakan lagi selepas pembetulan | 检测到系统时间异常，请校正后再继续使用 | システム時刻が異常です。修正後、使用してください |
...
| android | Network_error | Network error | Kesalahan jaringan | Ralat rangkaian | 网络错误 | ネットワークエラー |
...
| android | NO_LINK | NO LINK | TIDAK ADA TAUTAN | TIADA Pautan | 没有链接 | リンクがありません |
...
| android | Recaptcha_timeout | Recaptcha timeout | Batas waktu recaptcha | Tamat masa Recaptcha | 验证超时 | Recaptchaがタイムアウトしました |
...
| android | Retry_upload_failed | Retry upload failed. | Unggahan ulang gagal. | Gagal memuat naik semula. | 重新上传失败。 | アップロードの再試行に失敗しました。 |
...
| android | Share_error | Share error. | Bagikan kesalahan. | Kongsi ralat | 分享出错 | エラーを共有 |
...
| android | Text | Text |     |     | 文字  | テキスト |
| android | Theme | Theme | Tema | Tema | 主题  | テーマ |
...
| android | Type_message | Type message |     | Taipkan mesej | 输入消息 | メッセージを入力 |
...
| iOS | Asset_Type | Asset Type |     |     | 资产类型 | 資産タイプ |
...
| iOS | clear_group_chat_confirmation | Clear "%@" group chat? |     |     | 清除“%@”小组聊天？ | "%@"のグループチャットをクリアしますか? |
...
| iOS | error_network_task_failed | Network connection failed. Check or switch your network and try again |     |     | 网络连接失败。检查或切换网络，然后重试 | ネットワーク接続に失敗しました。ネットワーク接続状態を確認した後にもう一度試してください。 |
...
| iOS | Save_to_Camera_Roll | Save to Camera Roll |     |     | 保存到相册 | カメラロールに保存する |
| iOS | Saved | Saved |     |     | 已保存 | 保存しました |
| iOS | screen_lock_enable_biometric_hint | You’ll need to user %@ to unlock Mixin. you can still reply to messages from notifications and anwser calls if Mixin is locked. |     |     | 您需要使用%@来解锁 Mixin。当 Mixin 锁定时，您仍可以接听通话或在通知中回复消息。 | Mixinをアンロックするには%@が必要になります。Mixinがロックされている場合でも、通知されたメッセージへの返信や電話の応答は可能です。 |
...
| android, iOS | Add_participants | Add Participants | Tambahkan Peserta | Tambah Peserta | 添加成员 | メンバーを追加 |
| android, iOS | Add_Sticker | Add Sticker |     |     | 添加贴纸 | スタンプを追加する |
| android, iOS | Add_stickers | Add Stickers |     |     | 添加所有表情 | スタンプを追加 |
| android, iOS | Add_Tag | Add Tag | Tambahkan Tag | Tambah Tag | 点击添加 Tag（标签） | タグを追加する |
| android, iOS | Added | Added |     |     | 已添加 | 追加済 |
...
| android, iOS | Change | Change | Ubah | Ubah | 更改  | 変更  |
...
| android, iOS | Developer | Developer | Pengembang | Pemaju | 开发者 | 開発者情報 |
...
| android, iOS | Dismiss_as_Admin | Dismiss as Admin | Singkirkan admin | Ketepikan pentadbir | 撤销管理员身份 | サブリーダー権限を解除 |
| android, iOS | Done | Done | Selesai | Selesai | 完成  | 完了  |
...
| android, iOS | Enter_your_phone_number | Enter your phone number | Masukkan nomor ponsel Anda | Masukkan nombor telefon bimbit anda | 输入你的手机号码 | 電話番号を入力して下さい |
...
| android, iOS | error_connection_timeout | Network connection timeout, please try again | Batas waktu sambungan jaringan | Tamat masa sambungan rangkaian | 网络连接超时 | ネットワーク接続がタイムアウトしました |
...
| android, iOS | Make_group_admin | Make group admin | Jadikan admin grup | Buat pentadbir kumpulan | 设定为群组管理员 | サブリーダー権限を付与 |
...
| android, iOS | my_mixin_id | My Mixin ID: %1$s | ID Mixin saya: %1$s | ID Mixin Saya: %1$s | 我的 Mixin ID: %1$s |     |
...
| android, iOS | NO_FILE | NO FILE | TIDAK ADA FILE | TIADA FAIL | 没有文件 | ファイルがありません |
...
| android, iOS | NO_MEDIA | NO MEDIA | TIDAK ADA MEDIA | TIADA MEDIA | 没有媒体 | メディアがありません |
| android, iOS | No_Memo | No Memo | Tanpa Memo | Tiada Memo | 没有 Memo（备注） | メモなし |
| android, iOS | No_network_connection | No network connection | Tidak ada sambungan jaringan | Tiada sambungan rangkaian | 无网络连接 | ネットワーク接続がありません |
| android, iOS | NO_POST | NO POST | TIDAK ADA POSTINGAN | TIADA POST | 没有文章 | 投稿がありません |
...
| android, iOS | Reset_Link | Reset Link |     |     | 重置邀请链接 | リンクを取り消す |
...
| android, iOS | Send | Send | Kirim | Hantar | 发送  | 送る  |
...
| android, iOS | Send_Without_Sound | Send Without Sound |     |     | 静音发送 | 通知音を鳴らさずに送信する |
...
| android, iOS | setting_auth_search_hint | Mixin ID, Name | Mixin ID, Nama | Mixin ID, Nama | Mixin ID, 昵称 | Mixin ID, 名前 |
...
| android, iOS | Share | Share | Bagikan | Berkongsi | 分享  | 共有  |
| android, iOS | Share_Contact | Share Contact |     |     | 分享他的名片 | 連絡先を共有 |
| android, iOS | Share_Link | Share Link | Bagikan Tautan | Kongsi pautan | 分享邀请链接 | リンクをシェアする |
...
| android, iOS | Sign_with_phone_number | Sign in with phone number | Masuk dengan nomor telepon | Log masuk dengan nombor telefon | 通过手机号登录 | 電話番号でログイン |
...
| android, iOS | Terms_of_Service | Terms of Service | Ketentuan Layanan | Terma Perkhidmatan | 服务条款 | 利用規約 |
...
| android, iOS | To  | To  |     |     | 至   |     |
| android, iOS | Today | Today | Hari ini | Hari ini | 今天  | 今日  |
...
| android, iOS | User_not_found | User not found | Pengguna tidak ditemukan | Pengguna tidak ditemui | 找不到这个用户 | アップロード失敗 |
| android, iOS | value_now | value now %1$s |     | nilai sekarang %1$s | 价值 %1$s | 現在価格 %1$s |
| android, iOS | value_then | value then %1$s |     | nilai maka %1$s | 当时价值 %1$s | 当時の価格 %1$s |
...
| android, iOS | wallet_transfer_recent_tip | You have already transferred %3$s to %2$s %1$s. Do you want to transfer it again? | Anda telah mentransfer %3$s ke %2$s %1$s. Ingin mentransfernya lagi? | Anda telah memindahkan %3$s ke %2$s %1$s. Adakah anda mahu memindahkannya lagi? | 你已经在 %1$s 给 %2$s 转账了 %3$s，确认还要再转一次吗？ | あなたは%1$sに同額の送金を%2$sへ送っています。再度%3$sを送金しますか？ |
...
| android, iOS | wallet_withdrawal_recent_tip | You have already withdrawn %3$s to %2$s %1$s, Do you want to withdraw it again? | Anda telah menarik %3$s ke %2$s %1$s, Anda ingin menariknya lagi? | Anda telah menarik balik %3$s ke %2$s %1$s, Adakah anda mahu menariknya semula? | 你已经在 %1$s 提现 %3$s 到 %2$s，确认还要再提一次吗？ | あなたは%1$sに同額の出金を%2$sへ送っています。再度%3$sを出金しますか？ |
...
| android, iOS | Whats_your_name | What's your name? | Siapa nama Anda? | Siapa nama awak? | 你的名字? | お名前は何ですか？ |
...
| Desktop | Login to Mixin Messenger by QR Code | Login to Mixin Messenger by QR Code |     |     | 通过二维码登录 Mixin Messenger |     |
| Desktop |     | Open Mixin Messenger on your phone, scan the QR Code on the screen and confirm your login. |     |     | 打开手机上的 Mixin Messenger，扫描屏幕上的二维码，确认登录。 |     |
